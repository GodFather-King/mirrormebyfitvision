...
const clothingItems: ClothingItem[] = [
  { id: '1', name: 'Classic Blazer', brand: 'LUXE', price: '$289', image: blazerImg, fit: 'Perfect' },
  { id: '2', name: 'Essential Tee', brand: 'BASICS', price: '$49', image: tshirtImg, fit: 'Good' },
  { id: '3', name: 'Midi Dress', brand: 'ELEGANCE', price: '$159', image: dressImg, fit: 'Perfect' },
];
...
const ClothingCarousel = ({ onSelect, selectedId }: ClothingCarouselProps) => {
  ...
  return (
    <div className="glass-card p-4 space-y-4">
      ...
      <div className="relative">
        <div className="flex items-center gap-4 overflow-hidden">
          {clothingItems.map((item, index) => (
            ...
            >
              ...
              >
                <div className="aspect-square rounded-lg overflow-hidden bg-muted/30 mb-3">
                  <img 
                    src={item.image} 
                    alt={item.name}
        ...
        <button 
          onClick={prevSlide}
          className="absolute left-0 top-1/2 -translate-y-1/2 -translate-x-2 w-8 h-8 glass-card rounded-full flex items-center justify-center text-muted-foreground hover:text-foreground"
        >
          <ChevronLeft className="w-4 h-4" />
        </button>
        <button 
          onClick={nextSlide}
          className="absolute right-0 top-1/2 -translate-y-1/2 translate-x-2 w-8 h-8 glass-card rounded-full flex items-center justify-center text-muted-foreground hover:text-foreground"
        >
