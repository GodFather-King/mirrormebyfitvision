...
serve(async (req) => {
  ...
  try {
    ...
    // Models tried in order. Falls back when the primary returns 429.
    const modelChain = [
      ...
      for (let attempt = 0; attempt < 3; attempt++) {
        ...
        if (response.status === 429) {
          ...
          if (attempt < 2) {
            ...
            console.log(`Rate limited on ${model}, waiting ${wait}ms before retry`);
            await new Promise(r => setTimeout(r, wait));
            continue;
          }
          // exhausted retries on this model — try next model
          console.log(`Falling over from ${model} due to repeated 429s`);
          break;
        }
    ...
    if (!response || !response.ok) {
      ...
      throw new Error(`AI try-on failed: ${lastStatus} - ${lastErrorText}`);
    }
